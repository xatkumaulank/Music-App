...
class PlayTransition : Transition {
    ...
    override fun createAnimator(sceneRoot: ViewGroup, startValues: TransitionValues,
        ...
        val startBackground: Drawable = BitmapDrawable(sceneRoot.context.resources, startImage)
        ...
        val shrinkingView = addViewToOverlay(sceneRoot, startImage.width,
        ...
        val circleView = addViewToOverlay(sceneRoot, minRadius * 2, minRadius * 2,
                circleBackground)
        val circleStartX = (startLoc[0] - sceneRootLoc[0] +
                (startView.width - circleView.width) / 2).toFloat()
        val circleStartY = (startLoc[1] - sceneRootLoc[1] +
                (startView.height - circleView.height) / 2).toFloat()
        ...
        val circleEndX = (endLoc!![0] - sceneRootLoc[0] +
                (endView.width - circleView.width) / 2).toFloat()
        val circleEndY = (endLoc[1] - sceneRootLoc[1] +
                (endView.height - circleView.height) / 2).toFloat()
        val circlePath = pathMotion.getPath(circleStartX, circleStartY, circleEndX,
                circleEndY)
        val circleAnimator: Animator = ObjectAnimator.ofFloat(circleView, View.TRANSLATION_X,
                View.TRANSLATION_Y, circlePath)
        val growingView = addViewToOverlay(sceneRoot, endView.width,
                endView.height, shrinkingBackground)
