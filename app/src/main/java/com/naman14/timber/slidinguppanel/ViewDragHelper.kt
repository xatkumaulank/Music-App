...
class ViewDragHelper private constructor(context: Context, forParent: ViewGroup?, cb: Callback?) {
    ...
    private fun saveInitialMotion(x: Float, y: Float, pointerId: Int) {
        ...
        mInitialMotionX!![pointerId] = mLastMotionX?.get(pointerId)!!
        mLastMotionY?.set(pointerId, y)
        mInitialMotionY!![pointerId] = mLastMotionY?.get(pointerId)!!
    ...
    fun shouldInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val edgesTouched = mInitialEdgesTouched?.get(pointerId)
                if (edgesTouched != null) {
        ...
        when (action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (viewDragState == STATE_DRAGGING) {
                    ...
                    val idx = (x - (mLastMotionX?.get(activePointerId)!!)).toInt()
                    val idy = (y - (mLastMotionY?.get(activePointerId)!!)).toInt()
    ...
    private fun checkNewEdgeDrag(delta: Float, odelta: Float, pointerId: Int, edge: Int): Boolean {
        ...
        if ((mInitialEdgesTouched?.get(pointerId)!!) and edge != edge || mTrackingEdges and edge == 0 || (mEdgeDragsLocked?.get(pointerId)!!) and edge == edge || (mEdgeDragsInProgress?.get(pointerId))?.and(edge) == edge ||
        ...
        return (mEdgeDragsInProgress?.get(pointerId)!!) and edge == 0 && absDelta > touchSlop
    }
    ...
    fun checkTouchSlop(directions: Int, pointerId: Int): Boolean {
        ...
        val dx = (mLastMotionX?.get(pointerId)!!) - mInitialMotionX!![pointerId]
        val dy = (mLastMotionY?.get(pointerId)!!) - mInitialMotionY!![pointerId]
        if (checkHorizontal && checkVertical) {
            return dx * dx + dy * dy > touchSlop * touchSlop
        } else if (checkHorizontal) {
            return Math.abs(dx) > touchSlop
    ...
    fun isEdgeTouched(edges: Int, pointerId: Int): Boolean {
        return isPointerDown(pointerId) && (mInitialEdgesTouched?.get(pointerId)!!) and edges != 0
    }
